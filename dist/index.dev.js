...
module.exports = {
  ...
  configure: function configure(options) {
    ...
    if (options.account) {
      this.flags = this.flags.concat(["--etherbase", options.account, "--unlock", options.account, "--password", join(this.datadir, ".password")]);
      unlock = true;
      password = true;
    }
    ...
    if (unlock && !password) {
      this.flags = this.flags.concat(["--password", join(this.datadir, ".password")]);
    }

    this.configured = true;
    return this.flags;
  },
  listen: function listen(stream, label, listener) {
  ...
  stdout: function stdout(label, listener) {
    this.listen("stdout", label, listener);
  },
  stderr: function stderr(label, listener) {
    this.listen("stderr", label, listener);
  },
  trigger: noop,
  start: function start(flags, listeners, trigger) {
    var self = this;
  ...
  stop: function stop(callback) {
    ...
    if (this.proc !== null) {
      var closed = function closed(code) {
